...
namespace sdds {
   ...
   bool CC::valid(const char* name,unsigned long long number,short cvv,short month,short year) const {
      ...
      if (number< 4000000000000000ull) return false;
      if (number>4099999999999999ull) return false;
      if (cvv<100||cvv >999) return false;
      if (month<1 ||month>12) return false;
      if (year <24||year>32) return false;
      return true;
   }
   ...
   void CC::display() const {
      if (isEmpty()) {
         ...
         printLine(name,numberexp,Year,expMonth,cvv);
      }
   }
}
